...
class Domino {
    ...
    run(context, component, root, todo = () => {}) {
        ...
        if (this.oldVDOM) {
            ...
            todo(); // Exécute les tâches supplémentaires
        } else {
     ...
     */
    connectComponent(context, component, root, todo) {
        ...
        this.run({ ...context, ...this.store.state }, this.initialComponent, root, todo); // Exécute le composant avec l'état initial
    }
}
